...
pub async fn load_scanner_state(app_handle: AppHandle) -> Result<(), ()> {
    ...
    let file_explorer_tree = filer::file_list::list::<String>(false, None, Some(filer::types::FileKind::Directory))
        .into_file_explorer()
        .into_tree();
...
pub async fn start_scanner(
    ...
    let file_explorer_tree = { shared_state.inner().0.public.lock().await.file_explorer_tree.clone() };

    utils::update_public_state(&app_handle, None, Some(true)).await;
    utils::update_status(&app_handle, None, None, Some(state::ScannerStatusStep::Listing)).await;

    // List selected paths
    let paths = filer::file_explorer::FileExplorer::new(file_explorer_tree.clone()).into_checked_paths();
    ...
    utils::update_status(&app_handle, None, None, Some(state::ScannerStatusStep::Counting)).await;
    ...
    let total_files_length = paths
        ...
        .sum::<usize>();
    debug!("start_scanner()", "Number of files to scan: {}.", total_files_length);

    utils::update_status(&app_handle, None, None, Some(state::ScannerStatusStep::Starting)).await;
...
#[tauri::command]
pub async fn stop_scanner(app_handle: AppHandle, shared_state: State<'_, state::SharedScannerState>) -> Result<(), ()> {
    ...
    if let Some(child_mut) = child_mutex_guard.as_mut() {
        ...
        utils::update_status(&app_handle, None, None, Some(state::ScannerStatusStep::Stopping)).await;

        child_mut.kill().await.expect("Failed to kill scanner process.");
...
pub async fn toggle_file_explorer_node_check(
    ...
    let mut next_file_explorer =
        filer::file_explorer::FileExplorer::new(public_state_mutex_guard.file_explorer_tree.to_owned());
